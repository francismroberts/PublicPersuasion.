...
- **`dashboard.js` is the shared rendering engine for every page.** It reads `window.COURSE` from `data.js` and renders the checklist, hero, nav, sidebar, and hub. One file, six pages (`index.html` + `week1–5.html`) all load it.
...
2. Confirm `COURSE.items.length` is still 54 (or the new expected total) and that every item still has all five enrichment fields (`capture`, `date`, `whyOrder`, `effort`, `mode`).
3. Render each changed page in jsdom, dispatch `DOMContentLoaded`, confirm zero JS errors, and confirm the progress pill's item count matches what's expected for that page.
